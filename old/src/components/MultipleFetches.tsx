...
export const MultipleFetches = () => {
  const { response: postResponse, callAPI: callPostAPI } = useAPI();
  const { response: commentsResponse, callAPI: callCommentsAPI } = useAPI();
      ...
      {!postResponse.loading && (
        <div>
          ...
          {commentsResponse.error && (
            <div data-testid="fetch-error-comments">{commentsResponse.error}</div>
          )}
